...
from implem.calls import is_sat_varisat
...
class Problem3SAT:
    ...
    def stringify_occurence_sequence(self,occurences):
        if len(occurences) == 0:
            ...
            occ = occurences[0]
            return "        <synch>m -> l{}".format(occ)
        else:

            targets = "(" + ",".join(["l{}".format(x) for x in occurences]) + ")"
            return "        <synch>m -> " + targets
    ...
    def make_signature(self,membership_to_gen_path):
        f = open("{}{}.hsf".format(membership_to_gen_path, self.name), "w")
        f.write(self.stringify_signature())

    def make_interaction(self,membership_to_gen_path):
        f = open("{}{}.hif".format(membership_to_gen_path,self.name), "w")
        f.write(self.stringify_interaction())
    ...
    def is_SAT(self):
        ...
        (result,solution,_,_) = is_sat_varisat(temp_dir_path,self.name,1)
...
def make_membership_for_problems(problem_dir_path,membership_to_gen_path):
    print("translating 3SAT to multi-trace analysis for benchmark in {}".format(problem_dir_path))
    for filename in os.listdir(problem_dir_path):
        if filename.endswith(".cnf"):
            ...
            prob.make_signature(membership_to_gen_path)
